...
Verify Information After Reject Request
    [Arguments]    ${company_title_name}    ${company_name}    ${id_number}    ${address}    
    ...            ${postcode}    ${title_name}    ${customer_first_name}    ${customer_last_name}    
    ...            ${customer_email}    ${customer_phone}    ${customer_phone_extra}    ${link_full_register}    ${remark}
    ${pms_txt_request_detail_header}=    Replace String    ${pms_txt_request_detail_header}    {value}    ${rm['text_request_detail']}
    Wait Until Element Is Visible    ${pms_txt_request_detail_header}    timeout=${DEFAULT_TIMEOUT}
    Wait Until Element Is Visible    ${pms_txt_request_status}    timeout=${DEFAULT_TIMEOUT}
    ${request_status}=    Get Text    ${pms_txt_request_status}
    Should Be Equal    ${request_status}    ${rm.text_status['reject']}
...
Verify Information On Request Details Page [Individual]
    ...
    Should Be Equal As Strings    ${label_id_number} ${value_id_number}    ${rm['text_id_card_number']} ${JuristicID}
